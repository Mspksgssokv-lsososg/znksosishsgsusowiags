    ...
    for (const file of cmdFiles) {
        try {
            ...
            if (cmd.name) {
                ...
                // ✅ IMPORTANT FIX: onLoad auto run
                if (cmd.onLoad) {
                    try {
                        cmd.onLoad(bot);
                        console.log(`${c}♻️ onLoad executed: ${file}${r}`);
                    } catch (err) {
                        console.log(`${y}❌ onLoad error (${file}): ${err.message}${r}`);
                    }
                }
            }
        } catch (err) {
    ...
    if (fs.existsSync(eventPath)) {
        ...
        for (const file of eventFiles) {
            try {
                ...
                console.log(`${y}❌ Error in Event: ${file} -> ${err.message}${r}`);
            }
        }
    }
